...
class AppDelegate: UIResponder, UIApplicationDelegate, WCSessionDelegate {
    ...
    func session(_ session: WCSession, didReceiveUserInfo userInfo: [String : Any] = [:]) {
        // Create a Distraction and save it into core data
        
        let distraction = Distraction(context: persistentContainer.viewContext)
        
        guard let date = userInfo["date"] as? Date, let name = userInfo["name"] as? String else {
            return
        }
        
        distraction.name = name
        distraction.date = date as NSDate?
